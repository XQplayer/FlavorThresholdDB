...
export default function StructureEvidence({ apiUrl, cas, inchikey, compoundName, isEnglish, onStatusChange }) {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/structures/resolve?${query}`, { signal: controller.signal }).then(async response => ({ ok: response.ok, data: await response.json() })).then(({ ok, data }) => setPayload(previous => ({ ...previous, ...data, loading: false, requestFailed: !ok }))).catch(error => { if (error.name !== 'AbortError') setPayload(previous => ({ ...previous, loading: false, requestFailed: true })); });
  ...
  return <section className="structure-evidence" aria-label={isEnglish ? 'Protein structure evidence' : '蛋白结构证据'}>
    ...
    {payload.loading ? <p>{isEnglish ? 'Resolving structures from verified proteins…' : '正在从已核验蛋白解析结构…'}</p> : payload.requestFailed ? <p>{isEnglish ? 'Structure sources are temporarily unavailable.' : '结构来源暂时不可用。'} <button type="button" onClick={() => { setPayload(previous => ({ ...previous, loading: true })); setRetryNonce(value => value + 1); }}>{isEnglish ? 'Retry structure sources' : '重试结构来源'}</button></p> : <div className="structure-evidence-grid">
